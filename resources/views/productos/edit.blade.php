...
@section('cabecera', 'Editar Producto - ' . $producto1->nombre)
    ...
    <div class="flex justify-center">
        <div class="card w-96 shadow-2xl bg-base-100">
            <div class="card-body">
                {{-- Imagen --}}
                <div>
                    @if(file_exists('images/productos/producto_' . $producto1->id . '.jpg'))
                        <img src="{{ asset('images/productos/producto_' . $producto1->id . '.jpg') }}" alt="{{$producto1->nombre}}" class="rounded-t-lg h-40 w-full object-cover">
                    @else
                        <img src="{{ asset('images/productos/default.jpg') }}" alt="{{$producto1->nombre}}" class="rounded-t-lg">
                    @endif
                </div>
                <form action="{{route('productos.update', $producto1)}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                {{-- Nombre --}}
